...
export async function POST(request: Request) {
  try {
    const { channelName, answers } = (await request.json()) as {
      channelName?: string;
      answers?: BriefAnswer[];
    };
    ...
    const subjectName = fallback(channelName);
    ...
    await transporter.sendMail({
      ...
      subject: `YouTube intro brief — ${subjectName}`,
      text: answers.map(({ question, answer }) => `${question}\n${fallback(answer)}`).join("\n\n"),
      html: `
        <h2>YouTube intro brief — ${subjectName}</h2>
        ${answers
          .map(
            ({ question, answer }) =>

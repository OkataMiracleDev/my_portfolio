...
export async function POST(request: Request) {
  try {
    const { videoTitle, answers } = (await request.json()) as {
      videoTitle?: string;
      answers?: BriefAnswer[];
    };
    ...
    const subjectName = fallback(videoTitle);
    ...
    await transporter.sendMail({
      ...
      subject: `Long-form video brief — ${subjectName}`,
      text: answers.map(({ question, answer }) => `${question}\n${fallback(answer)}`).join("\n\n"),
      html: `
        <h2>Long-form video brief — ${subjectName}</h2>
        ${answers
          .map(
            ({ question, answer }) =>

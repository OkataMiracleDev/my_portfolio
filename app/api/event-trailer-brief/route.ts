...
export async function POST(request: Request) {
  try {
    const { eventName, answers } = (await request.json()) as {
      eventName?: string;
      answers?: BriefAnswer[];
    };
    ...
    const subjectName = fallback(eventName);
    ...
    await transporter.sendMail({
      ...
      subject: `Event trailer brief — ${subjectName}`,
      text: answers.map(({ question, answer }) => `${question}\n${fallback(answer)}`).join("\n\n"),
      html: `
        <h2>Event trailer brief — ${subjectName}</h2>
        ${answers
          .map(
            ({ question, answer }) =>

...
const Stack = () => {
  ...
  return (
    <div data-aos="fade-up" className='relative w-full mt-[7rem] md:mt-[9rem] lg:mt-[10rem] 2xl:mt-[9rem] flex flex-col justify-center items-center'>
      {/* background text */}
      ...
      <div className="z-10 backdrop-blur-sm bg-[#6c628f33] shadow-md shadow-gray-400 rounded-xl p-4 grid grid-cols-3 gap-4">
        {icons.map((Icon, i) => (
          ...
          >
            <Icon className={`text-2xl ${Icon === RiOpenaiFill ? "scale-90" : ""}`} />
          </div>
        ))}
      </div>
    </div>

      ...
      <div className="max-w-6xl mx-auto">
        <div className="relative">
          ...
          <div className="relative z-10 flex justify-center">
            <div className="w-full max-w-2xl overflow-hidden rounded-card border border-ink/10 bg-base-raised">
              <div className="flex items-center gap-2 border-b border-ink/10 px-4 py-2.5">
                <span className="h-2.5 w-2.5 rounded-full bg-accent-build/70" />
                <span className="h-2.5 w-2.5 rounded-full bg-ink/15" />
                <span className="h-2.5 w-2.5 rounded-full bg-ink/15" />
                <span className="ml-2 font-[family-name:var(--font-jetbrains-mono)] text-[10px] uppercase tracking-[0.14em] text-ink/40">
                  ~/stack --list
                </span>
              </div>
              ...
              <div
                ref={gridRef}
                className="grid grid-cols-3 gap-2 p-4 md:grid-cols-4 md:gap-3 md:p-8"
              >
                {icons.map(({ Icon, name }, i) => (
                  <button
                    ...
                    className={`stack-btn relative flex min-h-[70px] flex-col items-center justify-center gap-1 rounded-lg p-3 transition-all duration-200 ease-out md:min-h-[90px] md:rounded-xl md:p-4 ${
                      activeIndex === i
                        ? "scale-105 bg-accent-build text-ink shadow-[0_0_20px_color-mix(in_oklch,var(--color-accent-build)_60%,transparent)]"
